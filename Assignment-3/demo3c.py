...
#  Thresholds for recursive ncuts
T1 = 5
T2 = 0.2

# Uncomment to use new values 
# T1 = 4
# T2 = 2
...
for name, img in images.items():
    ...
    # Non-Recursive n-cuts w/ k=2
    labels_nc2 = n_cuts(affinity, k=2)
    axs[1].imshow(labels_nc2.reshape(M, N), cmap="viridis")
    axs[1].set_title("Non-Recursive n-cuts (k=2)")
    axs[1].axis("off")

    # Non-Recursive n-cuts w/ k=3
    labels_nc3 = n_cuts(affinity, k=3)
    axs[2].imshow(labels_nc3.reshape(M, N), cmap="viridis")
    axs[2].set_title("Non-Recursive n-cuts (k=3)")
    axs[2].axis("off")

    # Spectral clustering w/ k=2
